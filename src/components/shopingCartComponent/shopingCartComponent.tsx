...
import DeliveryModeComponentCartStories from "../deliveryModeComponentCart/deliveryModeComponentCart.stories";
...
const ShoppingCartComponent: FC<ShoppingCartComponentProps> = ({globalproductTotalAmount,globalitem,globaldeliveryFee,globalgrandTotal,globalhandlingFee, globalquantity}) => {
  ...
  useEffect(() => {
    try {
      ...
      const data: CartItem[] = JSON.parse(
        sessionStorage.getItem("cartItemCount") || "[]"
      );
      if (data.length>0) {
        setCartItems(data);
  ...
  useEffect(() => {
    ...
    setQuantity(objCartItems);
    globalquantity(objCartItems);
  }, [productQuantities,productTotalAmount,grandTotal,deliveryFee,handlingFee]);
  ...
  const handleAddedQuantityChange = async (input: { quantity: number; product: Product }) => {
      const cartResponse = await addCart({mobile:userdata?.mobile,item:input});
      debugger
  ...
  const handleRemoveQuantityChange = async (input: { quantity: number; product: Product }) => {
    const cartResponse = await removeCart({mobile:userdata?.mobile,item:input});
    debugger
    if(cartResponse.statusText === "OK"){
      const cartGetResponse = await getCart({"mobile":userdata?.mobile});
      if (cartGetResponse?.statusText === "OK" && cartGetResponse?.data?.cartitems?.product?.length > 0) {
    ...
    <div className="p-0">
          ...
          <div className="p-3">
            <div className="flex items-center w-full"><a style={{ textDecoration: "none" }} href="/" className="w-7"><img src="https://www.kpnfresh.com/_next/static/media/back-button-arrow.8ac29b56.svg" alt="Bananas" /></a>
              <div className="flex justify-between items-center flex-1 w-full"><div className="text-lg pl-2 font-medium">Shopping Cart</div>
  ...
  <span className="font-medium text-sm text-gray-800">
    Delivering in 11 minutes
      ...
      {productQuantities.length === 0 ? (
        <p className="text-gray-500  pt-[2px]">Your cart is empty.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1  pt-[2px]" style={{zIndex:"555"}}>
